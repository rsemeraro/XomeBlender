...
* Running InXalizer with minimum requirements:
        ...
        python inxalizer -f file.bam -l my_label -r MyRef.fasta -scn 2 -vn 50 -sa Branched
        
* Running InXalizer with target file:

        python inxalizer -f file.bam -l my_label -r MyRef.fasta -scn 2 -vn 50 -sa Branched -b MyTargetfile.bed

* Running InXalizer with CNVs:

        python inxalizer -f file.bam -l my_label -r MyRef.fasta -scn 2 -vn 50 -sa Branched -c 3,1000000

InXalizer can be run to generate CNV files only by using the *"CNV list"*  function, invoked as ```-cl```, and a reference sequence.

        python inxalizer -r ref.fa -cl cnv_list.txt
...
* Running Xome-Blender with minimum requirements:
        ...
        python xomeblender -f Control.bam,Subclone1.bam,Subclone2.bam -la my_label -sc 127 -p 30,40,30 -fc 110 -v Subclone1.vcf,Subclone2.vcf
* Running Xome-Blender with CNVs:

        python xomeblender -f Control.bam,Subclone1.bam,Subclone2.bam -la my_label -sc 127 -p 30,40,30 -fc 110 -v Subclone1.vcf,Subclone2.vcf -cnv my_label_CNV.txt
...
Alternatively, it can be run in *_"automated mode"_* by using the ```--list (-l)``` option. The activation of this parameter requires a list file, containing the info to run multiple consecutive analyses.
      ...
      python xomeblender -l list_file.txt
