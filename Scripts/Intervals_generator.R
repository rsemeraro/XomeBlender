...
PathIn <- split.vars[1]
Label <- split.vars[2]
EventSize <- split.vars[3]
EventsNumb <- split.vars[4]
ChrStyle <- split.vars[5]
FilesFolder <- split.vars[6]
IDs <- split.vars[7]
Ref <- split.vars[8]
Bed <- split.vars[9]
...
load(file.path(FilesFolder,"Scripts","ChrSizCentr.Rdata"))
...
if(!is.na(Bed)==TRUE)
{
  ...
  {
    BgzipString <- paste("bgzip -c", Bed, ">", BedGz, sep=" ")
